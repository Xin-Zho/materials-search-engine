...
| 4 | 每个合法标识均标准化后回填 | PASS 身份观测 620（列值）+ 301（uid 前缀自证）= 921，未解析 0 |
...
**921 个身份观测全部可解析到 620 条唯一 identifier —— 未解析 0，覆盖率 100.000%。**

口径必须分清，两者不可互换：

| 口径 | 数值 | 含义 |
|---|---|---|
| **唯一标准化身份数** | **620** | `paper_identifiers` 行数（PK `(id_type, normalized_value)`）。**这是唯一可以被称为「身份数量」的数字。** |
| **身份观测次数** | **921** | 620 条列值观测（`source ∈ papers.三列`）+ 301 条 uid 前缀自证观测（`source = paper_id_prefix`）。同一身份可被多通道观测到，故观测数 > 唯一数。 |

**禁写**：不得把 921 表述为「921 个唯一身份」——那会与 620 行直接冲突。
921 是**解析请求/观测计数**，620 才是**身份基数**。差额 301 全部落在已存在的 620 条上，未新增任何 identity。
...
再叠加一条本阶段新发现的前置：**先建立唯一身份写入入口，并让所有论文写入者必须经过它**。

> 表述纠正（2026-09-11 用户裁决）：不是「第一张新表解决写入错误」。
> 新表本身修不了任何东西——30 条错位是**写入者绕过校验**的产物。
> 正确的因果顺序是：**入口先存在 → 写入者被迫经过入口 → 入口内识别真实类型 → 错误不再产生**。
> 因此 P0-B 的前置不是「建表」，而是「收口写入路径」。详见
> `docs/2026-09-11-p0b1-identity-write-entry-design.md`。
